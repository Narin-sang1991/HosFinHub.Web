...
import type { InvoiceItemModel, InvoiceEditorModel } from "@/store/financial/paymentModel";
...
export function genarateAllCharges(invoiceItems: InvoiceItemModel[]) {
    ...
    allCharges.forEach((chargePrefix, i) => {
        ...
        if (invoiceInCharges.length > 0) {
            let overAmount: number = 0.00;
            let approvedAmount: number = 0.00;
...
export function getClaimStatusText(status: number) {
    let result: string = status == 0 ? 'ไม่มีรายการ'
        : status == 1 ? 'รอดำเนินการ'
            : status == 2 ? 'รอพิจารณา'
                : status == 3 ? 'อนุมัติแล้ว'
                    : status == 4 ? 'ไม่อนุมัติ' : defaultStrEmpty;
    return result;
}

export function getStatusDisplayType(status: number) {
    let result: any = status == 0 ? 'secondary'
        : status == 1 || status == 2 ? 'warning'
            : status == 3 ? 'success'
                : status == 4 ? 'danger' : 'secondary';
    return result;
}