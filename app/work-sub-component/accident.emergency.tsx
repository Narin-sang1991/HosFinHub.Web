...
const AccidentEmergencyTab = function AccidentEmergency({
  ...
  async function saveItem(key: React.Key): Promise<void> {
    ...
    tmpItems.splice(index, 1, {
      ...item,
      aedate: row.aedate ? dayjs(row.aedate, dateInterfaceFormat).format(dateInterfaceFormat) : '',
      aetime: row.aedate ? dayjs(row.aetime, timeInterfaceFormat).format(timeInterfaceFormat) : '',
      authae: row.authae,
      ucae: row.ucae,
      emtype: row.emtype,
      aetype: row.aetype,
  ...
  return (
    <React.Fragment>
      <Space direction="vertical" size="small" >
        <Form name="formEmergencyEditor" form={formEmergencyEditor} component={false} size="small">
          <Table
        ...
        <AccidentEmergencyEditor
          key='refer-in'
          accidentEmergency={referIn}
          isReferIn={true}
